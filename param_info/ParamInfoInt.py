    ...
    def validate(self,kk):
        ...
        if self.min != None and self.max != None:
            if kk == None or  kk < self.min or kk > self.max:
                return self.setError( ErrorCode.id_intRange , self._name, self.text, self.min , self.max )
        elif self.min != None:
            if kk == None or kk < self.min:
                return self.setError( ErrorCode.id_intMin , self._name, self.text, self.min )
        elif self.max != None:
            if kk == None or kk > self.max:
                return self.setError( ErrorCode.id_intMax , self._name, self.text, self.max )
        else:
            if kk == None:
                return self.setError( ErrorCode.id_int , self._name, self.text )
